def jumping_in_neighborhood(neighborhood):
    ...
    while not jump_cmd[0] == "Love!":
        ...
        if current_house + int(length_jump) > len(neighborhood) - 1:
            current_house = 0
        else:
            current_house += length_jump
        if not neighborhood[current_house] == 0:
    ...
    return [[i for i in neighborhood if not i == 0], current_house]
